...
class _ReserveState extends State<Reserve> {
  ...
  String _warning = '';
  List<String> availableDurations = <String>["01:00", "01:30", "02:00"];
  String _warning2 = '';
  String warning3 = "";
  ...
  @override
  void initState() {
    dbAdminReservations = firestore.collection('adminReservations').snapshots(
  ...
  void validateTime({required String hour, required String duration}) {
    ...
    if (_timeChosen != null) {
      ...
      print("dbStartTime: $dbStartTime");
      print("dbEndTime: $dbEndTime");
      print("pickedStartTime: $pickedStartTime");
      print("pickedEndTime: $pickedEndTime");
      if (dbEndTime == 0.0) {
        dbEndTime = 24.0;
      }
      if (dbEndTime == 0.5) {
  ...
  void _presentDatePicker() {
    ...
    showDatePicker(
      ...
      setState(() {
        _selectedDate = value;
        _timeChosen = null;
        _warning = 'Nenhuma Hora Escolhida!';
      });
    });
  }
    ...
    final TimeOfDay? newTime = await tPicker
      ...
      if (newTime != null) {
        setState(() {
          ...
          if (value!.hour == 0) {
            warning3 =
                "Atenção que a hora escolhida acontece na noite entre o dia ${_selectedDate!.day - 1} e o dia ${_selectedDate!.day}";
          } else {
            warning3 = "";
          }
          setTime(value);
...
// IF DATA CHOSEN IS AFTER NOW
        if (comparison == 1) {
          setState(() {
            _timeChosen = value;
            _isNotNow = true;
            // if(_timeChosen!.hour == 22 && _timeChosen!.minute<30){
            //   availableDurations = ["01:00", "01:30"];
            // }else if(_timeChosen!.hour == 22 && _timeChosen!.minute >= 30){
            //   availableDurations = ["01:00"];
            // }else if(_timeChosen!.hour == 23 && _timeChosen!.minute<30)
          });
...
//IF DAY IS NOT CHOSEN YET
  ...
  void _reserve() async {
    ...
    List<Map<String, dynamic>> reservationsToMake = [];
    String _thisDuration = _selectedDuration!;
    switch (_selectedDuration) {
      case "01:00":
        _thisDuration = "60";
    ...
    double intTime = _timeChosen!.hour+0.0;
    if(_timeChosen!.minute == 30 ){
      intTime += 0.5;
    }
    if(intTime+int.parse(_thisDuration)/60 >24){
      String _pin = randomNumbers();
    String _idd =
        "${DateFormat('ddMMyyyy').format(_selectedDate!)}$_timeChosen";
      Reservation _reservation = Reservation(
          ...
          duration: "23:59",
          state: 'por completar',
          userEmail: (chosenClient == null || !_asAnother)
              ? Userr().email
              : chosenClient!,
          completed: false,
          id: _idd,
      ...
      reservationsToMake.add({
        "id": _idd,
        "reservation":_reservation
      });

      String _idd2 =
          "${DateFormat('ddMMyyyy').format(_selectedDate!.add(Duration(days: 1)))}$_timeChosen";
      String dur = "";

      if(intTime+int.parse(_thisDuration)/60 == 24.5){
        dur = "00:30";
      }else if(intTime+int.parse(_thisDuration)/60 == 25){
        dur = "01:00";
      }else if(intTime+int.parse(_thisDuration)/60 == 25.5){
        dur = "01:30";
      }
      Reservation _reservation2 = Reservation(
          pin: _pin,
          cloneId: _pin,
          day: DateFormat('dd/MM/yyyy').format(_selectedDate!.add(Duration(days: 1))),
          hour: '00:00',
          duration: dur,
          state: 'por completar',
          userEmail: (chosenClient == null || !_asAnother)
    ...
    for (var res in reservationsToMake) {
      Reservation _reservation = res["reservation"];
      // String _idd =
      //     "${DateFormat('ddMMyyyy').format(_selectedDate!)}$_timeChosen";
      final day = reservations.child(res["id"]);
      reservationsToCheckOut.value.add(_reservation);
      try {
        //await reservations.set(_reservation);
        await day.set({
          'pin': _reservation.pin,
          'id': _reservation.id,
          'cloneId':_reservation.cloneId,
                    ...
                    Padding(
                      ...
                      child: Card(
                        ...
                        child: ListTile(
                          ...
                          onTap: _selectedDate == null? null : _presentTimePicker,
                        ),
                      ),
                    ),
                    Padding(
...
//DROPDOWN LIST TO CHOOSE DURATION ////////////////////////////////////////////////
                          child: DropdownButton<String>(
                            ...
                            items: availableDurations.map((String value) {
                            ...
                            onChanged: _timeChosen == null? null : (newValue) {
                              setState(
                                () {
                                  if (_timeChosen != null) {
                                    setState(() {
              ...
              if (Userr().role == "administrador")
                Row(
                  ...
                  children: [
                    Text(
                      AppLocalizations.of(context)!.reserveAsAnother,
                      style: const TextStyle(
                        fontSize: 16,
                      ),
                    ),
                    ...
                    onPressed: () async {
                      _activateListeners().then((_) {
                        if (_reservationValid &&
                          ...
                          setState(() {
                            _warning2 =
                                AppLocalizations.of(context)!.invalidSlot;
                          });
                          return;
...
//               if (_selectedDate != null && adminReservations.isNotEmpty &&
//                   adminReservations.any((element) =>
...
//                 ),
              StreamBuilder<QuerySnapshot>(
                ...
                builder: (BuildContext context,
                  ...
                  if (snapshot.data!.docs.isNotEmpty && _selectedDate != null) {
                    List<Widget> children = [];
                    for (DocumentSnapshot document in snapshot.data!.docs) {
                      Map<String, dynamic> data =
                          document.data()! as Map<String, dynamic>;
                      if (snapshot.data!.docs.isNotEmpty &&
                          !adminReservations.contains(data)) {
                        adminReservations.add(data);
                      }
                      if (_selectedDate != null &&
                          _selectedDate!.weekday == data["weekday"]) {
